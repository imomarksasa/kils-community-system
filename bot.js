...
client.on("message", (message) => {
  ...
  if (message.content.toLowerCase().startsWith(prefix + `help`)) {
    ...
    .setColor(0xCF40FA)
    .setDescription(`Hello! I'm Shop System, the Discord bot for super cool support ticket stuff and more! Here are my commands:`)
...
if (message.content.toLowerCase().startsWith(prefix + `new`)) {
    ...
    message.guild.createChannel(`ticket-${message.author.id}`, "text").then(c => {
        ...
        const embed = new Discord.RichEmbed()
        .setColor(0xCF40FA)
        .addField(`Hey ${message.author.username}!`, `**الرجاء الانتضـار الى ان يأتيك احد الادارة او احد البائـعين**`)
        .setTimestamp();
        c.send({ embed: embed });
    }).catch(console.error);
}
